...
namespace DAL
{
    public class EjercicioRepository : ConexionBD
    {
        ...
        public string ActualizarFotoEjercicioBD(string nombre, byte[] nuevaFoto)
        {
            ...
            try
            {
               ////// // conexionBd.Open();
                ...
                if (res == 0)
                {
                    return "Foto no actualizada";
                }
                return "Foto actualizada exitosamente";
            }
            catch (MySqlException ex)
            {
                return "Error al actualizar foto: " + ex.Message;
            }
            finally
            {
               //conexionBd.Close();
